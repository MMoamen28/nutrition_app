...
function Start-Foreground {
    ...
    & $py -m uvicorn website.nutrition_backend.main:app --host 127.0.0.1 --port $Port --log-level info 2>&1 | Tee-Object -FilePath $log -Append
}

function Start-Background {
    Write-Output "Starting uvicorn in background on port $Port (logging -> $log)"
    Start-Process -FilePath $py -ArgumentList '-m','uvicorn','website.nutrition_backend.main:app','--host','127.0.0.1','--port',$Port,'--log-level','info' -NoNewWindow -RedirectStandardOutput $log -RedirectStandardError $log
